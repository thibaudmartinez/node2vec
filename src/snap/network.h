...
public:
    ...
    public:
        ...
        TEdgeI(const TNodeI &NodeI, const TNodeI &EndNodeI, const int &EdgeN = 0) : CurNode(NodeI), EndNode(EndNodeI),
                                                                                    CurEdge(EdgeN) {}
...
public:
    ...
    public:
        ...
        int GetNbrNId(const int &EdgeN) const {
            return EdgeN < GetOutDeg() ? GetOutNId(EdgeN) : GetInNId(EdgeN - GetOutDeg());
        }
    ...
    public:
        ...
        TNodeI(const THashIter &NodeHIter, const TNodeEDatNet *NetPt) : NodeHI(NodeHIter),
                                                                        Net((TNodeEDatNet *) NetPt) {}
        ...
        TEdgeData &GetNbrEDat(const int &EdgeN) {
            return EdgeN < GetOutDeg() ? GetOutEDat(EdgeN) : GetInEDat(EdgeN - GetOutDeg());
        }
...
template<class TNodeData, class TEdgeData>
typename TNodeEDatNet<TNodeData, TEdgeData>::TEdgeI
TNodeEDatNet<TNodeData, TEdgeData>::GetEI(const int &SrcNId, const int &DstNId) const {
    const TNodeI SrcNI = GetNI(SrcNId);
    int NodeN = -1;
...
public:
    ...
    public:
        ...
        TEdge(const int &EId, const int &SourceNId, const int &DestNId, const TEdgeData &EdgeData) : Id(EId),
                                                                                                     SrcNId(SourceNId),
                                                                                                     DstNId(DestNId),
                                                                                                     EdgeDat(EdgeData) {}
    ...
    public:
        ...
        TNodeI(const THashIter &NodeHIter, const TNodeEdgeNet *NetPt) : NodeHI(NodeHIter),
                                                                        Net((TNodeEdgeNet *) NetPt) {}
    ...
    public:
        ...
        TEdgeI(const THashIter &EdgeHIter, const TNodeEdgeNet *NetPt) : EdgeHI(EdgeHIter),
                                                                        Net((TNodeEdgeNet *) NetPt) {}
...
template<class TNodeData, class TEdgeData>
int
TNodeEdgeNet<TNodeData, TEdgeData>::AddEdge(const int &SrcNId, const int &DstNId, int EId, const TEdgeData &EdgeDat) {
    if (EId == -1) {
        EId = MxEId;
        MxEId++;
    }
...
template<class TNodeData, class TEdgeData>
bool
TNodeEdgeNet<TNodeData, TEdgeData>::IsEdge(const int &SrcNId, const int &DstNId, int &EId, const bool &IsDir) const {
    ...
    for (int N = NodeH.FFirstKeyId(); NodeH.FNextKeyId(N);) {
        ...
        for (int e = 0; e < Node.GetOutDeg(); e++) {
            if (!IsEdge(Node.GetOutEId(e))) {
                const TStr Msg = TStr::Fmt("Out-edge id %d of node %d does not exist.", Node.GetOutEId(e),
                                           Node.GetId());
                if (ThrowExcept) { EAssertR(false, Msg); } else { ErrNotify(Msg.CStr()); }
                RetVal = false;
            }
...
public:
    ...
    public:
        ...
        TAIntI(const TIntVecIter &HIter, TStr attribute, bool isEdgeIter, const TNEANet *GraphPt) : HI(HIter), attr(),
                                                                                                    Graph(GraphPt) {
            isNode = !isEdgeIter;
            attr = attribute;
        }
    ...
    public:
        ...
        TAIntVI(const TIntVVecIter &HIter, const TIntHVecIter &HHIter, TStr attribute, bool isEdgeIter,
                const TNEANet *GraphPt, bool is_dense) : HI(HIter), IsDense(is_dense), HHI(HHIter), attr(),
                                                         Graph(GraphPt) {
            isNode = !isEdgeIter;
            attr = attribute;
        }

        TAIntVI(const TAIntVI &I) : HI(I.HI), IsDense(I.IsDense), HHI(I.HHI), attr(I.attr),
                                    Graph(I.Graph) { isNode = I.isNode; }

        TAIntVI &operator=(const TAIntVI &I) {
            HI = I.HI;
            HHI = I.HHI, Graph = I.Graph;
            isNode = I.isNode;
            attr = I.attr;
            return *this;
        }
    ...
    public:
        ...
        TAStrI(const TStrVecIter &HIter, TStr attribute, bool isEdgeIter, const TNEANet *GraphPt) : HI(HIter), attr(),
                                                                                                    Graph(GraphPt) {
            isNode = !isEdgeIter;
            attr = attribute;
        }
    ...
    public:
        ...
        TAFltI(const TFltVecIter &HIter, TStr attribute, bool isEdgeIter, const TNEANet *GraphPt) : HI(HIter), attr(),
                                                                                                    Graph(GraphPt) {
            isNode = !isEdgeIter;
            attr = attribute;
        }
...
public:
    ...
    explicit TNEANet(const int &Nodes, const int &Edges) : CRef(),
                                                           MxNId(0), MxEId(0), NodeH(), EdgeH(), KeyToIndexTypeN(),
                                                           KeyToIndexTypeE(), KeyToDenseN(), KeyToDenseE(),
                                                           IntDefaultsN(), IntDefaultsE(), StrDefaultsN(),
                                                           StrDefaultsE(),
                                                           FltDefaultsN(), FltDefaultsE(), VecOfIntVecsN(),
                                                           VecOfIntVecsE(),
                                                           VecOfStrVecsN(), VecOfStrVecsE(), VecOfFltVecsN(),
                                                           VecOfFltVecsE(), VecOfIntVecVecsN(), VecOfIntVecVecsE(),
                                                           VecOfIntHashVecsN(), VecOfIntHashVecsE(), SAttrN(),
                                                           SAttrE() { Reserve(Nodes, Edges); }
    ...
    TNEANet(const TNEANet &Graph) : MxNId(Graph.MxNId), MxEId(Graph.MxEId),
                                    ...
                                    VecOfIntVecVecsN(), VecOfIntVecVecsE(),
                                    VecOfIntHashVecsN(), VecOfIntHashVecsE(), SAttrN(), SAttrE() {}

    /// Constructor for loading the graph from a (binary) stream SIn.
    TNEANet(TSIn &SIn) : MxNId(SIn), MxEId(SIn), NodeH(SIn), EdgeH(SIn),
                         KeyToIndexTypeN(SIn), KeyToIndexTypeE(SIn), KeyToDenseN(SIn), KeyToDenseE(SIn),
                         IntDefaultsN(SIn), IntDefaultsE(SIn),
...
protected:
    TNEANet(const TNEANet &Graph, bool modeSubGraph) : MxNId(Graph.MxNId), MxEId(Graph.MxEId),
                                                       ...
                                                       KeyToIndexTypeE(Graph.KeyToIndexTypeE), KeyToDenseN(),
                                                       KeyToDenseE(Graph.KeyToDenseE),
                                                       IntDefaultsN(Graph.IntDefaultsN),
                                                       IntDefaultsE(Graph.IntDefaultsE),
                                                       StrDefaultsN(Graph.StrDefaultsN),
                                                       StrDefaultsE(Graph.StrDefaultsE),
                                                       FltDefaultsN(Graph.FltDefaultsN),
                                                       FltDefaultsE(Graph.FltDefaultsE),
                                                       VecOfIntVecsN(Graph.VecOfIntVecsN),
                                                       VecOfIntVecsE(Graph.VecOfIntVecsE),
                                                       VecOfStrVecsN(Graph.VecOfStrVecsN),
                                                       VecOfStrVecsE(Graph.VecOfStrVecsE),
                                                       VecOfFltVecsN(Graph.VecOfFltVecsN),
                                                       VecOfFltVecsE(Graph.VecOfFltVecsE),
                                                       VecOfIntVecVecsN(), VecOfIntVecVecsE(Graph.VecOfIntVecVecsE),
                                                       VecOfIntHashVecsN(),
                                                       VecOfIntHashVecsE(Graph.VecOfIntHashVecsE) {}

    TNEANet(bool copyAll, const TNEANet &Graph) : MxNId(Graph.MxNId), MxEId(Graph.MxEId),
                                                  NodeH(Graph.NodeH), EdgeH(Graph.EdgeH),
                                                  KeyToIndexTypeN(Graph.KeyToIndexTypeN),
                                                  KeyToIndexTypeE(Graph.KeyToIndexTypeE),
                                                  KeyToDenseN(Graph.KeyToDenseN), KeyToDenseE(Graph.KeyToDenseE),
                                                  IntDefaultsN(Graph.IntDefaultsN), IntDefaultsE(Graph.IntDefaultsE),
                                                  StrDefaultsN(Graph.StrDefaultsN), StrDefaultsE(Graph.StrDefaultsE),
                                                  FltDefaultsN(Graph.FltDefaultsN), FltDefaultsE(Graph.FltDefaultsE),
                                                  VecOfIntVecsN(Graph.VecOfIntVecsN),
                                                  VecOfIntVecsE(Graph.VecOfIntVecsE),
                                                  VecOfStrVecsN(Graph.VecOfStrVecsN),
                                                  VecOfStrVecsE(Graph.VecOfStrVecsE),
                                                  VecOfFltVecsN(Graph.VecOfFltVecsN),
                                                  VecOfFltVecsE(Graph.VecOfFltVecsE),
                                                  VecOfIntVecVecsN(Graph.VecOfIntVecVecsN),
                                                  VecOfIntVecVecsE(Graph.VecOfIntVecVecsE),
                                                  VecOfIntHashVecsN(Graph.VecOfIntHashVecsN),
                                                  VecOfIntHashVecsE(Graph.VecOfIntHashVecsE), SAttrN(Graph.SAttrN),
                                                  SAttrE(Graph.SAttrE) {}
    // virtual ~TNEANet() { }
public:
    ...
    void Save(TSOut &SOut) const {
        ...
        NodeH.Save(SOut);
        EdgeH.Save(SOut);
        KeyToIndexTypeN.Save(SOut);
        KeyToIndexTypeE.Save(SOut);
        KeyToDenseN.Save(SOut);
        KeyToDenseE.Save(SOut);
        IntDefaultsN.Save(SOut);
        IntDefaultsE.Save(SOut);
        StrDefaultsN.Save(SOut);
        StrDefaultsE.Save(SOut);
        FltDefaultsN.Save(SOut);
        FltDefaultsE.Save(SOut);
        VecOfIntVecsN.Save(SOut);
        VecOfIntVecsE.Save(SOut);
        VecOfStrVecsN.Save(SOut);
        VecOfStrVecsE.Save(SOut);
        VecOfFltVecsN.Save(SOut);
        VecOfFltVecsE.Save(SOut);
        VecOfIntVecVecsN.Save(SOut);
        VecOfIntVecVecsE.Save(SOut);
        VecOfIntHashVecsN.Save(SOut);
        VecOfIntHashVecsE.Save(SOut);
        SAttrN.Save(SOut);
        SAttrE.Save(SOut);
    }
    ...
    static PNEANet Load_V1(TSIn &SIn) {
        ...
        Graph->MxNId.Load(SIn);
        Graph->MxEId.Load(SIn);
        Graph->NodeH.Load(SIn);
        Graph->EdgeH.Load(SIn);
        Graph->KeyToIndexTypeN.Load(SIn);
        Graph->KeyToIndexTypeE.Load(SIn);
        Graph->IntDefaultsN.Load(SIn);
        Graph->IntDefaultsE.Load(SIn);
        Graph->StrDefaultsN.Load(SIn);
        Graph->StrDefaultsE.Load(SIn);
        Graph->FltDefaultsN.Load(SIn);
        Graph->FltDefaultsE.Load(SIn);
        Graph->VecOfIntVecsN.Load(SIn);
        Graph->VecOfIntVecsE.Load(SIn);
        Graph->VecOfStrVecsN.Load(SIn);
        Graph->VecOfStrVecsE.Load(SIn);
        Graph->VecOfFltVecsN.Load(SIn);
        Graph->VecOfFltVecsE.Load(SIn);
        return Graph;
    }
    ...
    static PNEANet Load_V2(TSIn &SIn) {
        ...
        Graph->VecOfFltVecsN.Load(SIn);
        Graph->VecOfFltVecsE.Load(SIn);
        Graph->VecOfIntVecVecsN.Load(SIn);
        Graph->VecOfIntVecVecsE.Load(SIn);
        Graph->SAttrN.Load(SIn);
        Graph->SAttrE.Load(SIn);
        return Graph;
    }
    ...
    int AddIntAttrDatN(const TNodeI &NodeI, const TInt &value, const TStr &attr) {
        return AddIntAttrDatN(NodeI.GetId(), value, attr);
    }
    ...
    int AddStrAttrDatN(const TNodeI &NodeI, const TStr &value, const TStr &attr) {
        return AddStrAttrDatN(NodeI.GetId(), value, attr);
    }
    ...
    int AddFltAttrDatN(const TNodeI &NodeI, const TFlt &value, const TStr &attr) {
        return AddFltAttrDatN(NodeI.GetId(), value, attr);
    }
    ...
    int AddIntVAttrDatN(const TNodeI &NodeI, const TIntV &value, const TStr &attr) {
        return AddIntVAttrDatN(NodeI.GetId(), value, attr);
    }
    ...
    int AppendIntVAttrDatN(const TNodeI &NodeI, const TInt &value, const TStr &attr) {
        return AppendIntVAttrDatN(NodeI.GetId(), value, attr);
    }
    ...
    int DelFromIntVAttrDatN(const TNodeI &NodeI, const TInt &value, const TStr &attr) {
        return DelFromIntVAttrDatN(NodeI.GetId(), value, attr);
    }
    ...
    int AddIntVAttrDatE(const TEdgeI &EdgeI, const TIntV &value, const TStr &attr) {
        return AddIntVAttrDatE(EdgeI.GetId(), value, attr);
    }
    ...
    int AppendIntVAttrDatE(const TEdgeI &EdgeI, const TInt &value, const TStr &attr) {
        return AppendIntVAttrDatE(EdgeI.GetId(), value, attr);
    }
...
public:
    ...
    /// Constructor that reserves enough memory for a network of Nodes nodes and Edges edges.
    explicit TUndirNet(const int &Nodes, const int &Edges) : MxNId(0), NEdges(0), SAttrN(), SAttrE() {
        Reserve(Nodes, Edges);
    }
    ...
    static PUndirNet Load_V1(TSIn &SIn) {
        PUndirNet Graph = PUndirNet(new TUndirNet());
        Graph->MxNId.Load(SIn);
        Graph->NEdges.Load(SIn);
        Graph->NodeH.Load(SIn);
        return Graph;
    }
    ...
    TEdgeI BegEI() const {
        TNodeI NI = BegNI();
        TEdgeI EI(NI, EndNI(), 0);
        if (GetNodes() != 0 && (NI.GetOutDeg() == 0 || NI.GetId() > NI.GetOutNId(0))) { EI++; }
        return EI;
    }
...
public:
    ...
    static PDirNet Load_V1(TSIn &SIn) {
        PDirNet Graph = PDirNet(new TDirNet());
        Graph->MxNId.Load(SIn);
        Graph->NodeH.Load(SIn);
        return Graph;
    }
